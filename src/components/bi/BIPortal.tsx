...
export const BIPortal: React.FC = () => {
  ...
  const [appliedSummaryFilters, setAppliedSummaryFilters] = useState({ startDate: '', endDate: '', channelId: '', status: 'all' as 'all' | 'complete' | 'incomplete' });
  ...
  const channelData = useMemo(() => (['email', 'whatsapp'] as ChannelType[]).map((channel) => {
  ...
  type ExportDataset = 'conversations' | 'messages' | 'users' | 'pages' | 'tags' | 'customer_emails' | 'waiting_queue' | 'audit_logs' | 'summaries';
  const exportDataset = (dataset: ExportDataset) => {
    if (dataset === 'conversations') return exportCsv([
      ['Conversation ID', 'Customer ID', 'Customer Name', 'Customer Email', 'Customer Phone', 'Channel', 'Status', 'Assigned Agent ID', 'Assigned Agent', 'Unread Count', 'Subject', 'Priority', 'Sentiment', 'Category/Tags', 'Created At', 'Last Message At', 'Last Message', 'Summary'],
      ...conversations.map((conversation) => [conversation.id, conversation.contactId, conversation.contact.name, conversation.contact.email || '', conversation.contact.phone || '', conversation.channelType, conversation.status, conversation.assignedAgentId || '', conversation.assignedAgent?.name || '', String(conversation.unreadCount || 0), conversation.subject || '', conversation.priority || '', conversation.sentiment || '', conversation.tags.map((tag) => tag.name).join(' | '), conversation.createdAt, conversation.lastMessageAt, conversation.lastMessageText, conversation.summary?.text || '']),
    ], 'conversations-email-whatsapp.csv');
    if (dataset === 'messages') return exportCsv([
      ['Message ID', 'Conversation ID', 'Channel', 'Sender Type', 'Sender ID', 'Sender Name', 'Message Type', 'Content', 'Created At', 'Delivery Status', 'External Message ID', 'Attachments JSON', 'Metadata JSON'],
      ...messages.map((message) => [message.id, message.conversationId, conversations.find((conversation) => conversation.id === message.conversationId)?.channelType || '', message.senderType, message.senderId, message.senderName || '', message.messageType, message.content, message.createdAt, message.metadata?.deliveryStatus || '', message.metadata?.externalMessageId || '', JSON.stringify(message.attachments || []), JSON.stringify(message.metadata || {})]),
    ], 'messages-email-whatsapp.csv');
    if (dataset === 'users') return exportCsv([['User ID', 'Name', 'Email', 'Role', 'Status', 'Status Started At', 'Created At'], ...users.map((user) => [user.id, user.name, user.email, user.role, user.status, user.statusStartedAt, user.createdAt])], 'users.csv');
    if (dataset === 'pages') return exportCsv([['Page ID', 'Name', 'Channel', 'Status', 'Settings JSON'], ...pages.filter((page) => page.channelType === 'email' || page.channelType === 'whatsapp').map((page) => [page.id, page.name, page.channelType, page.status, JSON.stringify(page.settings || {})])], 'pages-email-whatsapp.csv');
    if (dataset === 'tags') return exportCsv([['Tag ID', 'Name', 'Category', 'Color'], ...tags.map((tag) => [tag.id, tag.name, tag.category, tag.color])], 'tags.csv');
    if (dataset === 'customer_emails') return exportCsv([['Email ID', 'Conversation ID', 'Ticket Number', 'From Name', 'From Email', 'To Email', 'Subject', 'Body', 'Status', 'Priority', 'Received At', 'Is Read', 'Message ID', 'Attachments JSON'], ...customerEmails.map((email) => [email.id, email.conversationId || '', email.ticketNumber, email.fromName, email.fromEmail, email.toEmail, email.subject, email.body, email.status, email.priority, email.receivedAt, String(email.isRead), email.messageId || '', JSON.stringify(email.attachments || [])])], 'customer-emails.csv');
    if (dataset === 'waiting_queue') return exportCsv([['Queue ID', 'Conversation ID', 'Name', 'Email', 'Phone', 'Channel', 'Page Name', 'Message', 'Priority', 'Created At', 'Source Email ID', 'WhatsApp JID'], ...waitingQueue.map((item) => [item.id, item.conversationId || '', item.name, item.email, item.phone || '', item.channelType, item.pageName, item.message, item.priority || '', item.createdAt, item.sourceEmailId || '', item.whatsappJid || ''])], 'waiting-queue.csv');
    if (dataset === 'audit_logs') return exportCsv([['Audit ID', 'Action', 'Target Type', 'Target ID', 'User ID', 'Details', 'Created At'], ...auditLogs.map((log) => [log.id, log.action, log.targetType, log.targetId, log.userId, log.details, log.createdAt])], 'audit-logs.csv');
    return exportCsv([['Conversation ID', 'Channel', 'Customer', 'Summary', 'Customer Message Count', 'Last Customer Message', 'Last Customer Message At', 'Summary Updated At'], ...conversations.filter((conversation) => conversation.summary).map((conversation) => [conversation.id, conversation.channelType, conversation.contact.name, conversation.summary?.text || '', String(conversation.summary?.customerMessageCount || 0), conversation.summary?.lastCustomerMessage || '', conversation.summary?.lastCustomerMessageAt || '', conversation.summary?.updatedAt || ''])], 'conversation-summaries.csv');
  };

  const exportAllConversations = () => {
    const rows: string[][] = [['Dataset', 'Record ID', 'Channel', 'Customer/Name', 'Email', 'Status', 'Created/Received At', 'Updated/Last Message At', 'Details']];
    conversations.forEach((conversation) => rows.push(['conversations', conversation.id, conversation.channelType, conversation.contact.name, conversation.contact.email || '', conversation.status, conversation.createdAt, conversation.lastMessageAt, JSON.stringify({ assignedAgentId: conversation.assignedAgentId || null, unreadCount: conversation.unreadCount, summary: conversation.summary?.text || null })]));
    messages.forEach((message) => rows.push(['messages', message.id, conversations.find((conversation) => conversation.id === message.conversationId)?.channelType || '', message.senderName || message.senderType, '', message.metadata?.deliveryStatus || '', message.createdAt, message.createdAt, JSON.stringify({ conversationId: message.conversationId, content: message.content, messageType: message.messageType, attachments: message.attachments || [] })]));
    users.forEach((user) => rows.push(['users', user.id, '', user.name, user.email, user.status, user.createdAt, user.statusStartedAt, JSON.stringify({ role: user.role })]));
    pages.filter((page) => page.channelType === 'email' || page.channelType === 'whatsapp').forEach((page) => rows.push(['pages', page.id, page.channelType, page.name, '', page.status, '', '', JSON.stringify(page.settings || {})]));
    tags.forEach((tag) => rows.push(['tags', tag.id, '', tag.name, '', tag.category, '', '', JSON.stringify({ color: tag.color })]));
    customerEmails.forEach((email) => rows.push(['customer_emails', email.id, 'email', email.fromName, email.fromEmail, email.status, email.receivedAt, email.receivedAt, JSON.stringify({ subject: email.subject, ticketNumber: email.ticketNumber, preview: email.preview, attachments: email.attachments || [] })]));
    waitingQueue.forEach((item) => rows.push(['waiting_queue', item.id, item.channelType, item.name, item.email, 'waiting', item.createdAt, item.createdAt, JSON.stringify({ message: item.message, priority: item.priority || null })]));
    auditLogs.forEach((log) => rows.push(['audit_logs', log.id, '', log.action, '', '', log.createdAt, log.createdAt, JSON.stringify({ targetType: log.targetType, targetId: log.targetId, details: log.details })]));
    exportCsv(rows, 'bi-all-email-whatsapp-data.csv');
  };
  ...
  return <div className="flex-1 min-w-0 overflow-y-auto bg-slate-50 p-4 sm:p-6">
    <div className="mx-auto max-w-[1600px] space-y-5">
      <div className="rounded-2xl bg-slate-900 p-5 text-white shadow-lg"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-teal-300"><BarChart3 className="h-4 w-4" /> Business Intelligence</div><h1 className="mt-1 text-2xl font-black">Petbox Desk BI Portal</h1><p className="mt-1 text-xs text-slate-300">Read-only insights derived from live Agent Portal activity.</p></div><div className="flex flex-wrap gap-2"><button onClick={() => exportCsv([['Metric', 'Value'], ['Total queries', String(metrics.total)], ['Resolved', String(metrics.closed)], ['Resolution rate', `${metrics.resolutionRate}%`]], 'bi-summary.csv')} className="flex items-center justify-center gap-2 rounded-lg bg-teal-500 px-4 py-2 text-xs font-bold hover:bg-teal-400"><Download className="h-4 w-4" /> Export Summary</button><button type="button" onClick={exportAllConversations} className="flex items-center justify-center gap-2 rounded-lg bg-white/15 px-4 py-2 text-xs font-bold hover:bg-white/25"><Download className="h-4 w-4" /> Export All Data</button></div></div></div>
